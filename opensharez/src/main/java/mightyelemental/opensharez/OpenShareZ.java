...
public class OpenShareZ {
	...
	/** Tests if there is already an OpenShareZ process running and prevents another from opening */
	public static void stopIfAlreadyRunning() {
		AtomicInteger i = new AtomicInteger(0);
		ProcessHandle.allProcesses()
				.filter(ph -> ph.info().command().isPresent() && ph.info().commandLine().get().contains("OpenShareZ") && ph.info().commandLine().get().contains("java"))
				.forEach(( process ) -> {
					i.addAndGet(1);
				});
		if (i.get() > 1) {
	...
	public static void main( String[] args ) {
		...
		Utils.verifySupportForTransparency();
